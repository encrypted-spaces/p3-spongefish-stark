    ...
    F: Field + Unit + PartialEq + Send + Sync,
{
    ...
    fn preprocessed_trace(&self) -> Option<RowMajorMatrix<F>> {
        ...
        let vars_count = self.instance.vars_count;
        ...
        let public_multiplicities = public_multiplicities(&self.instance);
        ...
        for (row_idx, column) in ptrace.rows_mut().take(unpadded_rows).enumerate() {
            ...
            lookup.input_public = pair
                .input
                .map(|var| F::from_bool(public_multiplicities[var.index()].is_some()));
    ...
    P: Permutation<WIDTH, U = F>,
{
    ...
    for _ in 0..(target_len - witness_len) {
        let output = permutation.permute(&zero_input);
        witness
            .trace
            .push(QueryAnswerPair::new(zero_input.clone(), output));
    }
}
